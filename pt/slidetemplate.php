...
if (isset($_GET['curr_process_event']) ) {
	if( intval($_GET['curr_process_event']) == intval($_GET['total_event']) ) {
		header('Location: /pt/countdown/fb/cd1.php');
		exit();
		}
}
?>
...
$query = "SELECT * FROM template_kuliah WHERE status=1 AND tarikh BETWEEN CURDATE() AND DATE_ADD(CURDATE(),INTERVAL 7 DAY) ORDER BY tarikh, susunan";
$result = mysqli_query($conn, $query);
...
//				if( mSec <= 0 && nextday == 0 ) {
			...
			function move() {
				...
				if( curr_process_event < total_event ) {
					...
					window.location = 'slidetemplate.php?curr_process_event=' + curr_process_event + '&total_event=' + total_event;
				}
			    else window.location = '/pt/countdown/fb/cd1.php';
...
if ( $curr_process_event < $bil_event ) {
		...
		if($kuliah_show_slide==='Slide') {
			include('./templateslides/kuliahslides.php');
?>
...
<?php
 ...
 <img src="./templateslides/<?php echo $kuliah_file_template; ?>"/>
		...
		<?php
			switch($kuliah_file_template) {
			   case 'kuliah.jpg':
								include('./templateslides/kuliah.php');
								break;
			   case 'umum.jpg':
								include('./templateslides/umum.php');
								break;
			   case 'pengajian.jpg':
								include('./templateslides/pengajian.php');
								break;
								
								
			   case 'ceramah.jpg':
								include('./templateslides/ceramah.php');
								break;
								
			   case 'kelas.jpg':
								include('./templateslides/kelas.php');
								break;

			   case 'khutbah.jpg':
								include('./templateslides/khutbah.php');
								break;
			}	
