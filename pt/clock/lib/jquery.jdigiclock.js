...
(function($) {
    ...
    $.fn.getWeather = function(el) {
		...
		var strdate = "<p class = 'loading'>" + arg_date + "</p>"; // <?php  echo "<p class = 'loading'>16 November 2012</br>3 Safar 1434</p>"; ?>;
		
        el.find('#weather').html(strdate);
        el.find('#forecast_container').html('<p class="loading"></p>');
        ...
        $.getJSON('lib/proxy/' + proxy + '?location=' + el.weatherLocationCode + '&metric=' + el.weatherMetric, function(data) {
           ...
           el.find('#weather .loading, #forecast_container .loading').hide();
            ...
            el.find('#weather').html(strdate); //weather);  //akhirnya...
