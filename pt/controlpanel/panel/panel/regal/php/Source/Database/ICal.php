...
class ICal
{
     ...
     * Unless you know what you're doing, it should begin with "http://"
     *
     * @return Object The iCal Object
     */
            ...
            if (isset($anEvent['RRULE']) && $anEvent['RRULE'] != '') {
                ...
                if (isset($rrules['UNTIL'])) {
                    ...
                    if (in_array(
                        ...
                        for ($i = 1; $i <= $count; $i++) {
                            ...
                            $offset =
                                "{$day_ordinals[$day_number]} {$weekdays[$week_day]} of " .
                                $dtstart_clone->format('F Y H:i:01');
                            $dtstart->modify($offset);
                        }

                        // Jumping X months forwards doesn't mean the end date
                        // will fall on the same day defined in BYDAY
                ...
                switch ($frequency) {
                    case 'DAILY':
                        ...
                        while ($recurring_timestamp <= $until) {
                            ...
                            $is_excluded = array_filter(
                                $anEvent['EXDATE_array'],
                                function($val) use ($search_date) {
                    ...
                    case 'WEEKLY':
                        ...
                        $week_recurring_timestamp = (date('w', $start_timestamp) == 0) ?
                            $start_timestamp :
                            strtotime('last Sunday ' . date('H:i:s', $start_timestamp), $start_timestamp);
                        ...
                        while ($week_recurring_timestamp <= $until) {
                            ...
                            foreach ($weekdays as $day) {
                                ...
                                if (
                                    in_array($day, $bydays) &&
                                    $day_recurring_timestamp > $start_timestamp &&
                                    $day_recurring_timestamp <= $until
                                ) {
                                    // Add event to day
                    ...
                    case 'YEARLY':
                        ...
                        if (isset($rrules['BYDAY']) && $rrules['BYDAY'] != '') {
                            ...
                            while ($recurring_timestamp <= $until) {
                                ...
                                if (isset($rrules['BYMONTH']) && $rrules['BYMONTH'] != '') {
                                    $event_start_desc =
                                        "$day {$month_names[$rrules['BYMONTH']]} " .
                                        date('Y H:i:s', $recurring_timestamp);
                                } else {
                        ...
                        $events =
                            (isset
                                (
                                    $count_orig
                                ) && sizeof($events) > $count_orig) ? array_slice($events, 0, $count_orig) : $events;
                }
            }
        }
